...
router.post('/:id/claim/:claimId/accept', requireLogin, async (req, res) => {
  ...
  if (claim) {
    ...
    // Message to claimant
    const messageTextToClaimant = `Your claim for "${item.name}" was accepted. ${returnInfo}. ${returnBy}. If this is not yours, request a return within 72 hours and return the item within 5 days.`;
    await messageModel.createMessage({
      senderId: req.session.user.id,
      senderName: req.session.user.name,
    ...
    const messageTextToReporter = `Your claim for "${item.name}" has been accepted by ${claim.claimantName}. Returning person: ${claim.claimantName}. ${returnInfo}. ${contactMethod}`;
    await messageModel.createMessage({
      senderId: req.session.user.id,
      senderName: req.session.user.name,
      recipientId: item.userId,
